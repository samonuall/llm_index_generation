...
# ═════════════════════════════════════════════════════════════════════════════
# Unit 2: _classify (taxonomy keyword matching)
# ═════════════════════════════════════════════════════════════════════════════

class TestClassify:

    def test_title_prepend_is_context(self):
        assert CodeAgent._classify("Prepend title to all chunks") == "CONTEXT"

    def test_synonym_expansion_is_vocabulary(self):
        assert CodeAgent._classify("Add synonym expansion and abbreviation mapping") == "VOCABULARY"

    def test_sliding_window_is_structure(self):
        assert CodeAgent._classify("Sliding window chunking with overlap") == "STRUCTURE"

    def test_ngram_is_query_bridging(self):
        assert CodeAgent._classify("Extract n-gram phrases for BM25") == "QUERY-BRIDGING"

    def test_tfidf_is_query_bridging(self):
        assert CodeAgent._classify("TF-IDF term reweighting") == "QUERY-BRIDGING"

    def test_context_takes_priority_over_vocabulary(self):
        # "title" (CONTEXT keyword) + "synonym" (VOCABULARY keyword) → CONTEXT wins
        assert CodeAgent._classify("Prepend title and add synonym") == "CONTEXT"

    def test_unknown_description_defaults_to_vocabulary(self):
        assert CodeAgent._classify("Something completely unrelated") == "VOCABULARY"

    def test_case_insensitive(self):
        assert CodeAgent._classify("TITLE PREPEND STRATEGY") == "CONTEXT"
        assert CodeAgent._classify("SYNONYM EXPANSION") == "VOCABULARY"


# ═════════════════════════════════════════════════════════════════════════════
# Unit 3: Taxonomy diversity assignment logic
# ═════════════════════════════════════════════════════════════════════════════

class TestTaxonomyAssignment:

    def _get_assigned_categories(self, past_hypotheses: list, n: int = 4) -> list[str]:
        """Replicate the diversity assignment logic from generate_hypotheses."""
        taxonomy_categories = CodeAgent.TAXONOMY_CATEGORIES
        tried = [CodeAgent._classify(ph["description"]) for ph in past_hypotheses]
        unexplored = [c for c in taxonomy_categories if c not in tried]
        explored = [c for c in taxonomy_categories if c in tried]
        assigned = (unexplored + explored)[:n]
        while len(assigned) < n:
            assigned.append(taxonomy_categories[len(assigned) % len(taxonomy_categories)])
        return assigned

    def test_no_history_assigns_all_four_categories(self):
        assigned = self._get_assigned_categories(past_hypotheses=[])
        assert set(assigned) == {"VOCABULARY", "STRUCTURE", "CONTEXT", "QUERY-BRIDGING"}

    def test_unexplored_categories_come_first(self):
        # CONTEXT already tried — it should appear last in assignments
        past = [{"description": "Prepend title to chunks"}]  # CONTEXT
        assigned = self._get_assigned_categories(past, n=4)
        assert assigned[0] != "CONTEXT"
        assert "CONTEXT" in assigned  # still included, just deprioritised

    def test_all_categories_tried_still_assigns_four(self):
        past = [
            {"description": "Prepend title"},           # CONTEXT
            {"description": "Add synonym expansion"},    # VOCABULARY
            {"description": "Sliding window chunking"},  # STRUCTURE
            {"description": "Extract n-gram phrases"},   # QUERY-BRIDGING
        ]
        assigned = self._get_assigned_categories(past, n=4)
