...
class TestHypothesisAdoption:
    def test_best_hypothesis_selected(self):
        """Create 3 HypothesisResult objects, verify best delta is selected."""
        h1 = Hypothesis(id="H1", description="d1", rationale="r1", code="c1")
        h2 = Hypothesis(id="H2", description="d2", rationale="r2", code="c2")
        h3 = Hypothesis(id="H3", description="d3", rationale="r3", code="c3")

        results = [
            HypothesisResult(hypothesis=h1, delta_recall_100=0.05, proven=True),
            HypothesisResult(hypothesis=h2, delta_recall_100=0.12, proven=True),
            HypothesisResult(hypothesis=h3, delta_recall_100=0.08, proven=True),
        ]
        best = max(results, key=lambda r: r.delta_recall_100)
        assert best.hypothesis.id == "H2"
        assert best.delta_recall_100 == 0.12

    def test_no_adoption_when_all_negative(self):
        """All hypotheses have negative delta, verify none are proven."""
        h1 = Hypothesis(id="H1", description="d1", rationale="r1", code="c1")
        h2 = Hypothesis(id="H2", description="d2", rationale="r2", code="c2")
        h3 = Hypothesis(id="H3", description="d3", rationale="r3", code="c3")

        results = [
            HypothesisResult(hypothesis=h1, delta_recall_100=-0.05, proven=False),
            HypothesisResult(hypothesis=h2, delta_recall_100=-0.02, proven=False),
            HypothesisResult(hypothesis=h3, delta_recall_100=-0.10, proven=False),
        ]
        assert not any(r.proven for r in results)


class TestContrastiveMode:
    @patch("src.agents.analysis_code_agent.code_agent.completion")
    def test_query_lookup_built_when_enabled(self, mock_comp):
        """Verify contrastive markers appear in prompt when query_lookup is provided."""
        mock_comp.return_value = make_llm_response(
            content="### H1: test\nRationale: r\nQuery IDs: q1\nFalsifying: c\n```python\ncode\n```"
        )

        agent = CodeAgent(config={"api_base": "http://fake"})
        past_hypotheses = [{
            "id": "H0", "description": "old hyp", "delta_recall_100": 0.01,
            "delta_ndcg_10": 0.005, "proven": True, "notes": "",
            "improved_query_ids": ["q1"], "regressed_query_ids": ["q2"],
        }]
        query_lookup = {"q1": "furry pet animal", "q2": "planets orbiting sun"}
        ...
        called_msgs = mock_comp.call_args[1].get("messages") or mock_comp.call_args[0][0]
        prompt_text = " ".join(m["content"] for m in called_msgs if m.get("content"))
        assert "fixed" in prompt_text.lower() or "✓ fixed" in prompt_text
        assert "broke" in prompt_text.lower() or "✗ broke" in prompt_text


class TestHistoryMode:
    @patch("src.agents.analysis_code_agent.code_agent.completion")
    def test_past_hypotheses_included_when_enabled(self, mock_comp):
        """Verify past hypotheses appear in code agent prompt."""
        mock_comp.return_value = make_llm_response(
            content="### H1: test\nRationale: r\nQuery IDs: q1\nFalsifying: c\n```python\ncode\n```"
        )
        ...
        agent = CodeAgent(config={"api_base": "http://fake"})
        past_hypotheses = [{
            "id": "H0", "description": "chunk by paragraphs", "delta_recall_100": -0.02,
            "delta_ndcg_10": -0.01, "proven": False, "notes": "did not help",
        }]
        ...
        called_msgs = mock_comp.call_args[1].get("messages") or mock_comp.call_args[0][0]
        prompt_text = " ".join(m["content"] for m in called_msgs if m.get("content"))
        assert "Previously Tested Hypotheses" in prompt_text
        assert "chunk by paragraphs" in prompt_text
