...
def build_eval_prompt(
    ...
    if include_query_text:
        ...
        top_misses = misses[:5]
        worst_hits = sorted(hits, key=lambda x: x["rank"] or 0, reverse=True)[:5]
        ...
        missed_suffix = f" (showing {len(top_misses)} of {len(misses)})" if len(misses) > 5 else ""
        hit_suffix = f" (showing {len(worst_hits)} of {len(hits)})" if len(hits) > 5 else ""

        missed_section = (
            f"### Missed queries ({len(misses)} / {len(query_results)}){missed_suffix}:\n"
            + ("\n".join(missed_lines) if missed_lines else "  (none)")
        )
        hit_section = (
            f"### Retrieved queries ({len(hits)} / {len(query_results)}){hit_suffix} — sorted worst rank first:\n"
