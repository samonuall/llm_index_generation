...
A two-stage LLM agent that iteratively improves document preprocessing for
**dense retrieval** (Qwen3 embedding model + cosine similarity, served over an
OpenAI-compatible endpoint). Same architecture as `analysis_code_agent`, but
the retrieval backend is dense instead of BM25.

1. **Analysis stage** — an LLM investigates retrieval failures using tool calls (looking at documents, querying the dense-index server, reading eval results).
2. **Hypothesis stage** — a second LLM generates N candidate `preprocess.py` implementations, each is tested empirically against the live dense-index server, and the best-performing one is adopted.

## Configuring the embedding endpoint

Set the following keys in `config.yaml` to point at your Qwen3 embedding server
(any OpenAI-compatible endpoint exposing `POST /v1/embeddings` works — vLLM,
sglang, TEI, etc.):

```yaml
embedding_endpoint: "http://your-host:8001/v1"
embedding_model: "Qwen/Qwen3-Embedding-8B"
embedding_api_key: "EMPTY"            # or your real key
query_instruction: "Given a web search query, retrieve relevant passages that answer the query."
```

You can also override them via environment variables picked up by
`dense_server.py`: `EMBEDDING_ENDPOINT`, `EMBEDDING_MODEL`, `EMBEDDING_API_KEY`,
`QUERY_INSTRUCTION`.
...
```bash
# Single loop
uv run python main.py --agent analysis_code_agent_dense --loops 1

# Multiple loops with contrastive history
uv run python main.py --agent analysis_code_agent_dense --loops 3 --condition agent_contrastive

# One-shot dense baseline (no iterative loop)
uv run python main.py --agent one_shot_dense
```
...
├── eval_utils.py       # Subset eval utilities (dense server-based)
├── dense_server.py     # FastAPI server for in-memory dense (Qwen3) index management
├── dense_client.py     # HTTP client for the dense server
