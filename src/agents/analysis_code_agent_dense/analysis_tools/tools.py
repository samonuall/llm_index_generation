"""Structured tools for the AnalysisAgent (dense variant).
...
``src/agents/analysis_code_agent_dense/analysis_agent.py`` under
...
def vector_retrieve(client, query: str, top_k: int = 10) -> str:
    """Query the current dense vector index and return results as JSON.
    ...
    try:
        ...
        return f"Error running vector_retrieve: {e}"
...
TOOL_SCHEMAS = [
    {
        ...
        "function": {
            ...
            "description": (
                "Query the current dense vector index (LanceDB, HNSW, cosine similarity) "
                "to see what documents are retrieved for a given query. "
                "Returns doc_id, cosine-similarity score (in [-1, 1]; higher is more similar), and rank for each result."
            ),
            "parameters": {
                "type": "object",
                "properties": {
