"""Structured tools for the AnalysisAgent (dense retrieval variant).

Three tools are exposed via LiteLLM's OpenAI-compatible tool-calling API so the
LLM can investigate dense-retrieval failures without arbitrary shell access.

When tools change, make sure to update the prompt in
``src/agents/analysis_code_agent_dense/analysis_agent.py`` under the
``_build_initial_context()`` function.
"""
...
def dense_retrieve(
    client, query: str, top_k: int = 10, index_name: str = "current"
) -> str:
    """Query the currently-active dense LanceDB index and return results as JSON."""
    try:
        results = client.retrieve(name=index_name, query=query, top_k=top_k)
        return json.dumps(results)
    except Exception as e:
        return f"Error running dense_retrieve on '{index_name}': {e}"
...
TOOL_SCHEMAS = [
    {
        ...
        "function": {
            "name": "dense_retrieve",
            "description": (
                "Query the current dense (Qwen3 embedding) retrieval index to see "
                "what documents are retrieved for a given query. Returns doc_id, "
                "cosine-similarity score, and rank for each result."
            ),
            "parameters": {
                "type": "object",
                "properties": {
...
def dispatch_tool(
    ...
    ``current_index_name`` is the LanceDB table name for the current iteration's
    index (e.g. ``current_iter3``). It is injected so the LLM never has to know
    or pass the dynamic per-iteration name itself.
    """
    ...
    # Backwards-compat alias: some LLMs may still emit the old tool name.
    elif name == "bm25_retrieve":
        return dense_retrieve(
            client=client,
            query=args["query"],
            top_k=args.get("top_k", 10),
