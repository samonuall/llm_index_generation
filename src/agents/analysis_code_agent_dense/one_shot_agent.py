...
def run_one_shot(split: str = "tip_of_the_tongue", model: str | None = None, api_base: str | None = None, max_distractors: int = 9000) -> None:
    ...
    persist_dir = str(_AGENT_DIR / ".dense_cache")
    ...
    server_proc = subprocess.Popen(
        [
            ...
            "--embedding-endpoint", config.get("embedding_endpoint", "http://localhost:8001/v1"),
            "--embedding-model", config.get("embedding_model", "Qwen/Qwen3-Embedding-8B"),
            "--embedding-api-key", config.get("embedding_api_key", "EMPTY"),
            "--embedding-batch-size", str(config.get("embedding_batch_size", 32)),
            "--embedding-max-chars", str(config.get("embedding_max_chars", 20000)),
            "--query-instruction", config.get(
                "query_instruction",
    ...
    # Poll for readiness instead of sleeping a fixed time; dense server can take
    # longer to start than BM25 because it loads an embedding client.
    client = DenseClient(
        base_url=f"http://localhost:{server_port}",
        batch_size=config.get("dense_batch_size", 10_000),
    )
    max_wait = config.get("server_startup_timeout", 60)
    for i in range(max_wait * 2):
        if client.health():
            print(f"[one_shot] Dense server ready (took {(i+1)*0.5:.1f}s).")
            break
        time.sleep(0.5)
    else:
        raise RuntimeError(f"Dense server failed to start after {max_wait}s.")
    ...
    prompt = f"""## Task
You are given a DENSE retrieval system (Qwen3 embedding model + cosine similarity) over a document corpus.
