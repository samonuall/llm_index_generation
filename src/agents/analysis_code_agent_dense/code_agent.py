...
- Retrieval is DENSE: each chunk is embedded with a Qwen3 embedding model and queries are scored by cosine similarity against chunk embeddings (MaxP per doc). There is no tokenizer / stemmer / stopword list you need to work around — you only control what text goes into each chunk.
...
- The corpus has 200K+ docs. Splitting each into many small chunks creates millions of embeddings and shifts the candidate pool toward short, semantically generic fragments that can crowd out the gold doc. Keep additional chunks to 1-3 per document maximum.
- Chunks longer than ~20K chars are truncated before embedding — extremely long chunks waste tokens without adding signal. For very long sections, a summary-style or head-and-key-paragraphs chunk is more effective than a raw dump.
...
IMPORTANT NOTES:
- The documents in this dataset have EMPTY metadata dicts (no title, no aliases). Do NOT rely on doc.metadata for anything.
- Retrieval is DENSE: each chunk is embedded with a Qwen3 embedding model and queries are scored by cosine similarity against chunk embeddings (MaxP per doc). You only control what text goes into each chunk — there is no tokenizer / stemmer to work around.
- Documents are full-length documents (potentially thousands of words), not pre-chunked passages.
...
- The corpus has 200K+ docs. Avoid creating many small chunks per doc (keep to 1-3 additional chunks max) — over-chunking inflates the dense index with short, semantically-generic fragments that can crowd out the gold doc.
- Chunks longer than ~20K chars are truncated before embedding — favour focused sub-chunks (e.g. a plot summary or key-entity list) over raw concatenation of everything.
...
"""
    ...
    def test_hypothesis(
        ...
        """Test a single hypothesis by building a per-iteration LanceDB table
        and running a subset eval, comparing against ``current_index_name``.

        The hypothesis index name is ``hyp_iter{iteration}_{hypothesis.id}`` so
        every hypothesis tested in every loop gets its own persistent table.
        """
        from .eval_utils import load_preprocessor_from_code, run_subset_eval

        result = HypothesisResult(hypothesis=hypothesis)
        index_name = f"hyp_iter{iteration}_{hypothesis.id}"
        ...
        try:
            ...
            # Build hypothesis index on server (persisted as a LanceDB table)
            client.build_index(index_name, chunks, persist=True)

            # Run subset eval on hypothesis index
            hyp_eval = run_subset_eval(index_name, test_queries, client)

            # Run subset eval on the current iteration's index for comparison
        ...
        # Hypothesis LanceDB table is intentionally left on disk so embeddings
        # for every hypothesis / iteration can be inspected or re-evaluated.
        return result
