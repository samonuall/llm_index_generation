...
class BM25Index:
    def __init__(self, chunks: List[Chunk]) -> None:
        self._chunks = chunks
        corpus_tokens = bm25s.tokenize([c.text for c in chunks])
        self._retriever = bm25s.BM25()
        self._retriever.index(corpus_tokens)

    def search(self, query: str, top_k: int = 10) -> List[Tuple[Chunk, float]]:
        query_tokens = bm25s.tokenize([query])
        results, scores = self._retriever.retrieve(query_tokens, k=min(top_k, len(self._chunks)))
        return [(self._chunks[int(idx)], float(score)) for idx, score in zip(results[0], scores[0])]

    def search_documents(self, query: str, top_k: int = 10) -> List[Tuple[str, float]]:
        chunk_results = self.search(query, top_k=len(self._chunks))
        doc_scores = {}
